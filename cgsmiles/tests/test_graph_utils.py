...
@pytest.mark.parametrize('cgsmiles_str, bond_ops', (
                        # simple string
                        ("{[#SC2][#SC2][#SP2]}.{#SC2=[$]CCC[$],#SP2=[$]CCO}", {0: ["$01"], 10: ["$01"], 12: ["$11"], 19: ["$11"]}),
                        # simple string bond order larger than 1
                        ("{[#SC2][#SC2]}.{#SC2=[$]=CCC}", {0: ["$02"], 9: ["$02"]}),
                        # simple with ring
                        ("{[#TC5]1[#TC5][#TC5]1}.{#TC5=[$]cc[$]}", {0: ["$01"], 1: ["$11"], 4: ["$01"], 5: ["$21"], 8: ["$11"], 9: ["$21"]}),
                        # simple ring with bond order
                        ("{[#SC3]=[#SC3]}.{#SC3=[$]CCC[$]}", {0: ["$01"], 2:["$11"], 9:["$01"], 11:["$11"] }),
                        # simple ring with squash operator
                        ("{[#SC4]1[#TC5][#TC5]1}.{#SC4=Cc(c[!])c[!],#TC5=[!]ccc[!]}", {5: ["!01"], 7: ["!11"], 11: ["!21"]}),
                        # zero
                        ("{[#A][#B]}.{#A=CC[O-].[$],#B=[$].[Na+]}", {2: ["$00"], 8: ["$00"]}),
                        # connect between squashed atom and other fragment
                        #("{[#TP1]1[#SC3A][#SC3]1}.{#SC3A=[$][!]CCCC,#SC3=CCCC[!],#TP1=[$]O}", {}),
))
