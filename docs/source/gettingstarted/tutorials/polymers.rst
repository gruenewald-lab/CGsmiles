...
.. code:: python
  ...
  ax, pos = draw_molecule(res_graph,
                          colors={node: 'orange' for node in res_graph.nodes},
                          cg_mapping=False,
                          labels=labels)
  plt.show()
...
.. code:: python
  ...
  # Using 2 resolutions
  cgsmiles_str_two = "{[#PMA]([#PEG][#PEG][#PEG])|5}.{#PMA=[<]CC[>]C(=O)OC[$],#PEG=[$]COC[$]}"

  # Using 3 resolutions
  cgsmiles_str_three = "{[#mPEG]|5}.{#mPEG=[$][#PMA][$]([#PEG][#PEG][#PEG])}.{#PMA=[<]CC[>]C(=O)OC[$],#PEG=[$]COC[$]}"
